...
\[ p_{\lambda} = \sum_{\mu \geq \lambda} \alpha_{\mu} m_{\mu} \qquad \textrm{and} \qquad e_{\lambda} = \sum_{\mu \leq \lambda^t} \beta_{\mu} m_{\mu} \quad \textrm{for coefficients $\alpha_{\mu},\beta_{\mu}\in\mathbb{C}$}\]
where the $\leq$ is dominance order (and recall that $\lambda^t$ denotes the \emph{transpose} of $\lambda$).

\item Let $\lambda \vdash n$ and define $f^{\lambda}$ to be the coefficient of $x_1x_2\cdots x_n$ in the Schur function $s_{\lambda}$. Explain why $f^{\lambda} = f^{\lambda^t}$, and also give an example showing that this is not true for other coefficients.
